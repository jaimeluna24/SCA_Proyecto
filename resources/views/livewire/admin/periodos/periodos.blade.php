<div>
        ...
        <form wire:submit.prevent="register()" class="w-10/12 mx-auto mt-8 ">
            <div class="grid md:grid-cols-3 md:gap-6 md:mb-6">
                ...
                <div class="mb-3">
                    ...
                    <input placeholder="2024" wire:model="anio" type="text" id="first_name"
                        class="{{ $errors->has('anio') ? ' border-red-600' : '' }} bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        required />
                    <div class="errors">
                        @error('anio')
                            {{ $message }}
            ...
            <div class="grid md:grid-cols-2 md:gap-6 md:mb-6">
                <div class="mb-3">
                    <label for="first_name"
                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Fecha de inicio</label>
                    <input wire:model="fecha_inicio" type="date" id="first_name"
                        class="{{ $errors->has('fecha_inicio') ? ' border-red-600' : '' }} bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        required />
                    <div class="errors">
                        @error('fecha_inicio')
                            {{ $message }}
                ...
                <div class="mb-3">
                    <label for="first_name"
                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Fecha de final</label>
                    <input wire:model="fecha_final" type="date" id="first_name"
                        class="{{ $errors->has('fecha_final') ? ' border-red-600' : '' }} bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        required />
                    <div class="errors">
                        @error('fecha_final')
                            {{ $message }}
    ...
    @elseif($deshabilitar)
        <div
            ...
            <p class="text-sm font-normal text-gray-500 dark:text-gray-400"> ¿Estás seguro de deshabilitar el periodo
                {{ $periodoName }}?</p>
            ...
            <div class="flex mt-7">
                @if ($periodo->active == 1)
                <button wire:click="eliminarPeriodo()" type="button"
                class="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">Deshabilitar</button>

                @else
                <button wire:click="eliminarPeriodo()" type="button"
                class="focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-900">Habilitar</button>
            ...
            <form class="w-10/12 mx-auto mt-8 ">
                <div class="grid md:grid-cols-3 md:gap-6 md:mb-6">
                    ...
                    <div class="mb-3">
                        <label for="first_name"
                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Identificador</label>
                        <input wire:model="identificador" type="text" id="first_name"
                            class="{{ $errors->has('identificador') ? ' border-red-600' : '' }} bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                            required />
                        <div class="errors">
                            @error('identificador')
                    ...
                    <div class="mb-3">
                        <label for="first_name"
                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Año</label>
                        <input wire:model="anio" type="text" id="first_name"
                            class="{{ $errors->has('anio') ? ' border-red-600' : '' }} bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                            required />
                        <div class="errors">
                            @error('anio')
            ...
            <tbody>
                @forelse ($periodos as $item)
                    <tr class="bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <td class="px-6 py-4">{{ $item->identificador }}</td>
                        <td class="px-6 py-4">{{ $item->anio }}</td>
                        <td class="px-6 py-4">{{ $item->fecha_inicio }}</td>
                        <td class="px-6 py-4">{{ $item->fecha_final }}</td>
                        ...
                        <td class="px-3 py-2">
                            ...
                            wire:click="modeDeshabilitar({{ $item->id }})">Deshabilitar</button>
                        <button  class="focus:outline-none text-white bg-yellow-700 hover:bg-yellow-800 focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-yellow-600 dark:hover:bg-yellow-700 dark:focus:ring-yellow-900" wire:click="modeEdit({{ $item->id }})">
                            ...
                            wire:click="modeDeshabilitar({{ $item->id }})">Habilitar</button>
                        <button  class="focus:outline-none text-white bg-yellow-700 hover:bg-yellow-800 focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-yellow-600 dark:hover:bg-yellow-700 dark:focus:ring-yellow-900" wire:click="modeEdit({{ $item->id }})">
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24"><g fill="none" stroke="#ffffff" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"><path d="m16.475 5.408l2.117 2.117m-.756-3.982L12.109 9.27a2.118 2.118 0 0 0-.58 1.082L11 13l2.648-.53c.41-.082.786-.283 1.082-.579l5.727-5.727a1.853 1.853 0 1 0-2.621-2.621"/><path d="M19 15v3a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V7a2 2 0 0 1 2-2h3"/></g></svg>
                        </button>
                            @endif
